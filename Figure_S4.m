...
for k = 1:2
    [t, sol, spikeTimesModel] = run_case(P, cases(k));
    nexttile;
    plot_case(t, sol, cases(k).title);
    results(k).t = t;
    results(k).sol = sol;
    results(k).spikeTimesModel = spikeTimesModel;
...
function [t, sol, spikeTimesModel] = run_case(P, caseDef)
P.from1 = caseDef.from1;
P.to1 = caseDef.to1;
P.from2 = caseDef.from2;
P.to2 = caseDef.to2;
P.stim_target = caseDef.stim_target;
...
output(1)=(-iNaA-iKA-iCaLA-iCaTA-iSKA-iHA-iLA+stimA)/P.CX;
...
output(71)=(-iNaB-iKB-iCaLB-iCaTB-iSKB-iHB-iLB+stimB)/P.CX;
