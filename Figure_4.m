function results = Figure_4()
P = get_common_params();

tspan = 0:0.01:1500;
delayVals = [50 150 200 280];
nDelays = numel(delayVals);

numSpikes = zeros(1,nDelays);
iCaT_CSN_all = zeros(numel(tspan), nDelays);
iH_CSN_all   = zeros(numel(tspan), nDelays);
peakICaT = zeros(1,nDelays);
peakIH   = zeros(1,nDelays);
...
for k = 1:nDelays
    ...
    onsetB = onsetA + durationA + d;

    P.from1 = onsetA;
    P.to1   = onsetA + durationA;
    P.from2 = onsetB;
    P.to2   = onsetB + durationB;

    [t,sol] = ode23(@(t,sol) FTN_Network_P_local(t,sol,P), tspan, P.init_cond);
    ...
    aTinf = 1 ./ (1 + exp((VCSN - P.thetaaT) / P.sigmaaT));
    bTinf = 1 ./ (1 + exp((rTCSN - P.thetab) / P.sigmab)) ...
          - 1 ./ (1 + exp(-P.thetab / P.sigmab));

    denom = 1 - exp((2*VCSN) / P.RTF);
    small = abs(denom) < eps;
    denom(small) = eps * sign(denom(small) + (denom(small)==0));

    iCaT_CSN = P.gCaTCSN .* (aTinf.^3) .* (bTinf.^2) .* VCSN .* (P.Ca_ex ./ denom);
    iH_CSN   = P.gHCSN .* (P.krX*rfCSN + (1-P.krX).*rsCSN) .* (VCSN - P.VH);

    iCaT_CSN_all(:,k) = iCaT_CSN;
    iH_CSN_all(:,k)   = iH_CSN;

    gapMask = (t >= P.to1) & (t <= P.from2);
    afterBMask = (t >= P.from2) & (t <= P.from2 + 100);

    if any(afterBMask)
        peakICaT(k) = max(iCaT_CSN(afterBMask));
    else
        peakICaT(k) = NaN;
    end

    if any(gapMask)
        peakIH(k) = max(iH_CSN(gapMask));
    else
        peakIH(k) = NaN;
    end

    spikeTimesModel = spike_times_local(VCSN, 0.4);
    numSpikes(k) = numel(spikeTimesModel);
end

magICaT = abs(peakICaT);
magIH   = abs(peakIH);

normICaT = magICaT / max(magICaT);
normIH   = magIH   / max(magIH);
...
hold on
for k = 1:nDelays
    plot(tspan, iH_CSN_all(:,k), 'LineWidth', 1.2, 'Color', cols(k,:));
end
xlabel('Time (ms)');
ylabel('I_H');
title('I_H across delays');
legend(arrayfun(@(d)sprintf('%g ms',d), delayVals, 'UniformOutput', false), ...
       'Location','best');
xlim([0 800]);
box off
hold off
...
hold on
for k = 1:nDelays
    plot(tspan, iCaT_CSN_all(:,k), 'LineWidth', 1.2, 'Color', cols(k,:));
end
xlabel('Time (ms)');
ylabel('I_{CaT}');
title('I_{CaT} across delays');
legend(arrayfun(@(d)sprintf('%g ms',d), delayVals, 'UniformOutput', false), ...
       'Location','best');
xlim([0 800]);
box off
...
nexttile([1 2])
yyaxis left
plot(delayVals, numSpikes, '-o', 'LineWidth', 1.8, 'MarkerSize', 7, 'Color', 'k');
ylabel('CSN spike count');
set(gca, 'YColor', 'k');
ylim([0, max(numSpikes)+1]);

for k = 1:nDelays
    text(delayVals(k), numSpikes(k)+0.12, sprintf('%d', numSpikes(k)), ...
         'HorizontalAlignment','center', 'FontSize',8);
end

yyaxis right
plot(delayVals, normICaT, '-s', 'LineWidth', 1.8, 'MarkerSize', 7);
hold on
plot(delayVals, normIH, '-^', 'LineWidth', 1.8, 'MarkerSize', 7);
ylabel('Normalized peak current magnitude');
set(gca, 'YColor', 'k');
ylim([0, 1.1]);

xlabel('Inter-stimulus delay (ms)');
title('CSN spikes and intrinsic current magnitudes versus delay');
legend({'Spikes','|I_{CaT}|','|I_H|'}, 'Location','best');
xlim([min(delayVals)-10, max(delayVals)+10]);
box off

title(tl, 'Figure 4');

results.delayVals = delayVals;
results.tspan = tspan;
results.iCaT_CSN_all = iCaT_CSN_all;
results.iH_CSN_all = iH_CSN_all;
results.peakICaT = peakICaT;
results.peakIH = peakIH;
results.numSpikes = numSpikes;
results.normICaT = normICaT;
results.normIH = normIH;
end
...
P.gCaTCSN = 0.5; P.gSKCSN = 2; P.gHCSN = 6;
