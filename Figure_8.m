function results = Figure_8()

P = get_common_params();

durations = linspace(10,200,10);
magnitudes = linspace(10,300,10);

durationA_default = 50;
durationB_default = 50;
Iapp1_default = 200;
Iapp2_default = 200;
delay_default = 50;
onsetA = 100;

numSpikesS1_duration = zeros(size(durations));
numSpikesS2_duration = zeros(size(durations));
numSpikes_duration_heatmap = zeros(numel(durations), numel(durations));

numSpikesS1_magnitude = zeros(size(magnitudes));
numSpikesS2_magnitude = zeros(size(magnitudes));
numSpikes_magnitude_heatmap = zeros(numel(magnitudes), numel(magnitudes));

for i = 1:numel(durations)
    durA = durations(i);
    onsetB = onsetA + durA + delay_default;

    P.Iapp1 = Iapp1_default;
    P.Iapp2 = Iapp2_default;
    P.from1 = onsetA;
    P.to1 = onsetA + durA;
    P.from2 = onsetB;
    P.to2 = onsetB + durationB_default;

    [~, sol] = ode23(@(t,sol) FTN_Network_P_local(t,sol,P), P.tspan_long, P.init_cond);
    VCSN = sol(:,64);
    spikeTimesModel = spike_times_local(VCSN, 0.4);
    numSpikesS1_duration(i) = numel(spikeTimesModel);
end

for i = 1:numel(durations)
    durB = durations(i);
    onsetB = onsetA + durationA_default + delay_default;

    P.Iapp1 = Iapp1_default;
    P.Iapp2 = Iapp2_default;
    P.from1 = onsetA;
    P.to1 = onsetA + durationA_default;
    P.from2 = onsetB;
    P.to2 = onsetB + durB;
    ...
    VCSN = sol(:,64);
    spikeTimesModel = spike_times_local(VCSN, 0.4);
    numSpikesS2_duration(i) = numel(spikeTimesModel);
end
...
for i = 1:numel(durations)
    for j = 1:numel(durations)
        ...
        VCSN = sol(:,64);
        spikeTimesModel = spike_times_local(VCSN, 0.4);
        numSpikes_duration_heatmap(i,j) = numel(spikeTimesModel);
    end
end
...
for i = 1:numel(magnitudes)
    ...
    P.from1 = onsetA;
    P.to1 = onsetA + durationA_default;
    P.from2 = onsetA + durationA_default + delay_default;
    P.to2 = P.from2 + durationB_default;

    [~, sol] = ode23(@(t,sol) FTN_Network_P_local(t,sol,P), P.tspan_long, P.init_cond);
    VCSN = sol(:,64);
    spikeTimesModel = spike_times_local(VCSN, 0.4);
    numSpikesS1_magnitude(i) = numel(spikeTimesModel);
end
...
for i = 1:numel(magnitudes)
    ...
    VCSN = sol(:,64);
    spikeTimesModel = spike_times_local(VCSN, 0.4);
    numSpikesS2_magnitude(i) = numel(spikeTimesModel);
end
...
for i = 1:numel(magnitudes)
    for j = 1:numel(magnitudes)
        ...
        VCSN = sol(:,64);
        spikeTimesModel = spike_times_local(VCSN, 0.4);
        numSpikes_magnitude_heatmap(i,j) = numel(spikeTimesModel);
    end
end
...
nexttile
hold on
plot(durations, numSpikesS1_duration, 'o-', 'LineWidth', 1.5, 'Color', [0.46,0.01,0.65]);
plot(durations, numSpikesS2_duration, 'o-', 'LineWidth', 1.5, 'Color', [0.61,0.62,0.67]);
box off
xlabel('Duration (ms)')
ylabel('Number of spikes generated by CSN')
legend({'S1 Duration', 'S2 Duration'}, 'Location', 'best')
title('(A)')
hold off

nexttile
imagesc(durations, durations, numSpikes_duration_heatmap)
set(gca,'YDir','normal')
xlabel('S2 duration (ms)')
ylabel('S1 duration (ms)')
title('(B)')
colorbar
box off
...
nexttile
hold on
plot(magnitudes, numSpikesS1_magnitude, 'o-', 'LineWidth', 1.5, 'Color', [0.46,0.01,0.65]);
plot(magnitudes, numSpikesS2_magnitude, 'o-', 'LineWidth', 1.5, 'Color', [0.61,0.62,0.67]);
box off
xlabel('Magnitude (pA)')
ylabel('Number of spikes generated by CSN')
legend({'S1 Magnitude', 'S2 Magnitude'}, 'Location', 'best')
title('(C)')
hold off

nexttile
imagesc(magnitudes, magnitudes, numSpikes_magnitude_heatmap)
set(gca,'YDir','normal')
xlabel('S2 magnitude (pA)')
ylabel('S1 magnitude (pA)')
title('(D)')
colorbar
box off
...
results.numSpikesS1_duration = numSpikesS1_duration;
results.numSpikesS2_duration = numSpikesS2_duration;
results.numSpikes_duration_heatmap = numSpikes_duration_heatmap;
results.numSpikesS1_magnitude = numSpikesS1_magnitude;
results.numSpikesS2_magnitude = numSpikesS2_magnitude;
results.numSpikes_magnitude_heatmap = numSpikes_magnitude_heatmap;
