...
conds(1) = struct('gHCSN',6,  'gCaTCSN',0.4,  'color',[0.95,0.19,0.19], 'label','gH = 6, gCaT = 0.4');
conds(2) = struct('gHCSN',20, 'gCaTCSN',0.4,  'color',[0.27,0.50,0.25], 'label','gH = 20, gCaT = 0.4');
conds(3) = struct('gHCSN',6,  'gCaTCSN',0.05, 'color',[0.00,0.73,0.79], 'label','gH = 6, gCaT = 0.05');

results = struct([]);

for k = 1:numel(conds)
    Pk = P;
    Pk.gHCSN = conds(k).gHCSN;
    Pk.gCaTCSN = conds(k).gCaTCSN;

    [t, sol] = ode23(@(t,sol) FTN_Network_P_local(t,sol,Pk), Pk.tspan, Pk.init_cond);

    results(k).t = t;
    results(k).sol = sol;
    results(k).gHCSN = Pk.gHCSN;
    results(k).gCaTCSN = Pk.gCaTCSN;
    results(k).label = conds(k).label;
end

figure('Color','w');
hold on

VINTA1 = results(1).sol(:,43);
VINTA2 = results(1).sol(:,50);
VB     = results(1).sol(:,71);

plot(results(1).t, VINTA1, 'Color', [0.53,0.53,0.53], 'LineWidth', 1);
plot(results(1).t, VINTA2, 'Color', [0.53,0.53,0.53], 'LineWidth', 1);
plot(results(1).t, VB,     'Color', [0.79,0.79,0.79], 'LineWidth', 1);

h = gobjects(numel(conds),1);
for k = 1:numel(conds)
    VCSN = results(k).sol(:,64);
    h(k) = plot(results(k).t, VCSN, 'Color', conds(k).color, 'LineWidth', 1);
end

legend(h, {conds.label}, 'TextColor', 'black', 'Location', 'best')
xlabel('Time (msec)')
ylabel('Membrane Potential (mV)')
axis([50 500 -100 60])
title('CSNs Intrinsic currents interplay')
box off
hold off
...
P.gCaTCSN = 0.4; P.gSKCSN = 2; P.gHCSN = 6;
...
if isempty(set_crossgi) < 1
    clear set_cross_plusgi set_cross_minus
